...
check("going down on a clean floor is right", isCorrect(false, "down"));
check("going up on a wrong floor is right", isCorrect(true, "up"));
check("going down on a wrong floor is not", !isCorrect(true, "down"));
check("going up on a clean floor is not", !isCorrect(false, "up"));

// Neither button is ever the safe one, or the game plays itself.
{
  const always = (call: "up" | "down") =>
    [true, false].filter((anomalous) => isCorrect(anomalous, call)).length;
  check("neither call is always right", always("up") === 1 && always("down") === 1);
}

check("a right call goes one deeper", judge(2, false, "down").depth === 3);
check("a wrong call costs the whole run", judge(4, false, "up").depth === 0,
  `landed at depth ${judge(4, false, "up").depth}`);
check("and puts the player back on the reference floor",
  judge(4, false, "up").floor === REFERENCE_FLOOR);
...
// A run of right calls has to actually finish.
{
  ...
  while (steps < 50) {
    ...
    const call = spec.anomaly ? "up" : "down";
...
// The run must be winnable on any hotel, not just this one.
{
  ...
  for (let i = 0; i < 200; i += 1) {
    ...
    for (let step = 0; step < 20 && depth < DEPTH_TO_WIN; step += 1) {
      ...
      depth = judge(depth, spec.anomaly !== null, spec.anomaly ? "up" : "down").depth;
    }
